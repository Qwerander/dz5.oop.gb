...
public class StudentService {
    public Student createStudent(String name, String lastName, int idgroup) {// отвечает только за создание студента
                                                                             // (чистый метод)
        int id;
    ...
    public Student getById(int id) throws Exception {
        ...
        if (student == null) {
            throw new Exception("Студент не найден");// выбрасываем проверяемое исключение, которое компилятор требует
                                                     // отработать
        }
        return student;
    }
